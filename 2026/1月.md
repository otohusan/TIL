...
使用したブランチを **古い → 新しい** の順で取得して特定(逆順の場合は `-committerdate`)

```
git branch --sort=committerdate
```

直前のブランチとかに戻りたい場合だと以下の方が便利かも

```
git switch -
```

使うか分からないけど２個前とかに戻りたい場合は以下

```
git switch @{-2}
```

Raycast なんかにスニペットとして準備したり、エイリアス登録したりしたら便利そう。

### 429 エラー（Too Many Requests）

API などに短期間で過剰なリクエストを送信した場合に起こる。

## 1 月 29 日

### postgres order by

- 指定した式によって、要素を並び替えて取得することができる
- DESC, ASC の指定によって並び順を指定可能
- 複数の式を設定可能であり、同値があれば、左から右の順に式を探索していき、並び順を決める
- 最終的に全て同値の場合は、実装依存の順序(不定)な並びで返る
